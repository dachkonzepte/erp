...
from ..models import Employee, EmployeeAbsence, PlanningHoliday
...
def get_planning_capacity_settings(db: Session = Depends(get_db)):
...
def put_planning_capacity_settings(payload: PlanningSettingsUpdate, db: Session = Depends(get_db)):
...
def sync_planning_school_holidays(start_year: int | None = None, years: int = 3, force: bool = False, db: Session = Depends(get_db)):
...
def list_planning_holidays(db: Session = Depends(get_db)):
...
def create_planning_holiday(payload: PlanningHolidayCreate, db: Session = Depends(get_db)):
...
def update_planning_holiday(holiday_id:int, payload:PlanningHolidayCreate, db:Session=Depends(get_db)):
...
def delete_planning_holiday(holiday_id:int, db:Session=Depends(get_db)):
    row=db.get(PlanningHoliday,holiday_id)
    if row is None: raise HTTPException(status_code=404,detail="Feiertag / betriebsfreier Tag nicht gefunden.")
...
def list_employee_absences(start:date|None=None,end:date|None=None,employee_id:int|None=None,db:Session=Depends(get_db)):
...
def create_employee_absence(payload:EmployeeAbsenceCreate,db:Session=Depends(get_db)):
...
def update_employee_absence(absence_id:int,payload:EmployeeAbsenceCreate,db:Session=Depends(get_db)):
...
def get_planning_suggestion(payload:PlanningSuggestionRequest,db:Session=Depends(get_db)):
    try: return planning_suggestion(db,**payload.model_dump())
    except ValueError as exc: raise HTTPException(status_code=422,detail=str(exc)) from exc


@router.get("/api/planning")
def get_planning_board(start: date, end: date, db: Session = Depends(get_db)):
    try:
...
def create_planning_slot(payload: PlanningSlotCreate, db: Session = Depends(get_db)):
    try:
...
def put_planning_slot(slot_id: int, payload: PlanningSlotUpdate, db: Session = Depends(get_db)):
    try:
...
def remove_planning_slot(slot_id: int, db: Session = Depends(get_db)):
    try:
