...
Seit 1.0.9: die vormals lokale _require_admin()-Hilfsfunktion wurde durch
die zentrale Depends(require_admin(...))-Dependency aus app/deps.py ersetzt
(siehe README, "Berechtigungen vereinheitlicht"). Die Fehlermeldung bei
fehlender Admin-Rolle ist unverändert dieselbe wie zuvor.
"""
...
_ADMIN_ONLY = "Nur Administratoren dürfen das Zeiterfassungs-Backoffice verwenden."


@router.get("/api/time-backoffice/settings", response_model=TimeTrackingSettingsOut)
def get_time_backoffice_settings(db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
...
def put_time_backoffice_settings(payload: TimeTrackingSettingsUpdate, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
    try:
...
def get_work_time_models(db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
    return list_work_time_models(db)


@router.post("/api/time-backoffice/work-time-models")
def create_work_time_model(payload: WorkTimeModelUpsert, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
    try:
...
def put_work_time_model(model_id: int, payload: WorkTimeModelUpsert, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
    try:
...
def remove_work_time_model(model_id: int, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
...
def get_work_time_assignments(db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
    return employee_model_rows(db)


@router.put("/api/time-backoffice/work-time-assignments/{employee_id}")
def put_work_time_assignment(employee_id: int, payload: EmployeeWorkTimeModelUpdate, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
    try: return set_employee_model(db,employee_id,payload.model_id)
    except ValueError as exc: raise HTTPException(status_code=422,detail=str(exc)) from exc


@router.post("/api/time-backoffice/work-time-assignments/bulk")
def put_work_time_assignments_bulk(payload: EmployeeWorkTimeModelBulkUpdate, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
...
def get_time_payroll_employees(db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
...
def put_time_payroll_employee(employee_id: int, payload: EmployeePayrollSettingsUpdate, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
...
def get_time_backoffice_summary(start_date: date, end_date: date, employee_id: int | None = None, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
    return backoffice_summary(db,start_date,end_date,employee_id)


@router.get("/api/time-backoffice/timesheet.pdf")
def export_time_sheet_pdf(start_date: date, end_date: date, employee_id: int | None = None, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
...
def export_time_sheet_csv(start_date: date, end_date: date, employee_id: int | None = None, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
...
def export_datev_payroll(start_date: date, end_date: date, db: Session = Depends(get_db), _admin=Depends(require_admin(_ADMIN_ONLY))):
