...
def get_tax_keys(include_archived: bool = False, db: Session = Depends(get_db)):
...
def post_tax_key(payload: TaxKeyCreate, db: Session = Depends(get_db)):
    ensure_default_tax_keys(db)
...
def put_tax_key(key_id: int, payload: TaxKeyUpdate, db: Session = Depends(get_db)):
...
def post_set_default_tax_key(key_id: int, db: Session = Depends(get_db)):
...
def post_archive_tax_key(key_id: int, db: Session = Depends(get_db)):
    try:
...
def post_unarchive_tax_key(key_id: int, db: Session = Depends(get_db)):
