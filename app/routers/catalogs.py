...
def get_catalogs(include_archived: bool = Query(default=False), db: Session = Depends(get_db)):
    return list_catalogs(db, include_archived=include_archived)


@router.post("/api/catalogs", response_model=CatalogOut)
def post_catalog(payload: CatalogCreate, db: Session = Depends(get_db)):
...
def archive_catalog(catalog_id: int, db: Session = Depends(get_db)):
    try:
...
def unarchive_catalog(catalog_id: int, db: Session = Depends(get_db)):
